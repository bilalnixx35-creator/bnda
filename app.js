/* =========================
   BNDA SAFE DEBUG WRAPPER
   ========================= */

(() => {
  if (window.__BNDA_WRAPPER__) return;
  window.__BNDA_WRAPPER__ = true;

  const stamp = () => new Date().toISOString().slice(11, 19);
  const log = (...a) => console.log(`[${stamp()}] BNDA:`, ...a);

  log("app.js loaded ✅", location.href);

  // Catch crashes (so script never fails silently)
  window.addEventListener("error", (e) => {
    const msg = e?.message || "Unknown error";
    log("ERROR ❌", msg, e?.filename, e?.lineno, e?.colno);
    alert("BNDA ERROR:\n" + msg);
  });
  ...
  window.addEventListener("unhandledrejection", (e) => {
    ...
    alert("BNDA PROMISE ERROR:\n" + msg);
  });

  // Wait helper (prevents running before DOM/React UI is ready)
  function waitFor(checkFn, timeoutMs = 15000, label = "condition") {
  ...
  async function main() {
    ...
    if (document.readyState !== "complete") {
      log("waiting for document complete…");
      await waitFor(() => document.readyState === "complete", 15000, "document.readyState==complete");
    }
    log("document complete ✅");

    // Body exists
    await waitFor(() => document.body, 15000, "document.body");
    log("body ok ✅");

    // Prevent double-run
    if (window.__BNDA_APP_RUNNING__) {
      log("already running, skip ✅");
      return;
    }
    window.__BNDA_APP_RUNNING__ = true;

    log("starting your script now…");

    /* ==========================================================
       ✅ PASTE YOUR ORIGINAL CODE (IIFE) BELOW THIS LINE
       Remove nothing from your original code when pasting.
       ========================================================== */

    // >>> BNDA Leaderboard Script v2.1 <<<
  ...
  main().catch((err) => {
    log("WRAPPER FAILED ❌", err.message);
    alert("BNDA WRAPPER FAILED:\n" + err.message);
  });
})();
